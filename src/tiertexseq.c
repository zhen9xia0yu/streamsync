...
#include "tiertexseq.h"

int seq_probe(const AVProbeData *p)
{
    int i;
...
int seq_init_frame_buffers(SeqDemuxContext *seq, AVIOContext *pb)
{
    int i, sz;
...
int seq_fill_buffer(SeqDemuxContext *seq, AVIOContext *pb, int buffer_num, unsigned int data_offs, int data_size)
{
    TiertexSeqFrameBuffer *seq_buffer;
...
int seq_parse_frame_data(SeqDemuxContext *seq, AVIOContext *pb)
{
...
int seq_read_close(AVFormatContext *s)
{
    int i;
...
int seq_read_header(AVFormatContext *s)
{
    int i, rc;
    SeqDemuxContext *seq = s->priv_data;
    AVIOContext *pb = s->pb;
...
int seq_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    int rc;
    SeqDemuxContext *seq = s->priv_data;
    AVIOContext *pb = s->pb;
